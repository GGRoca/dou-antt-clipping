#!/usr/bin/env python3
"""
Script para backfill histórico sem envio de emails.
...
def run_for_date_no_email(cfg_path: str, run_date: date) -> int:
    """
    Versão do run_for_date que NUNCA envia email.
    """
    ...
    for f in new_files:
        ...
        try:
            ...
            print(f"❌ Erro processando {f.url}: {repr(e)}", file=sys.stderr)
            storage.log_run(
                run_date=run_date.isoformat(),
...
def main():
    p = argparse.ArgumentParser(description="Backfill histórico sem envio de emails")
    ...
    print(f"🔄 Iniciando backfill de {d1} até {d2}")
    print(f"📧 E-mail: DESABILITADO para backfill\n")
    ...
    for d in _daterange(d1, d2):
        total_days += 1
        print(f"\n{'='*60}")
        print(f"📅 Processando: {d.isoformat()}")
        print(f"{'='*60}")
    ...
    print(f"📊 Dias processados: {total_days}")
    print(f"📊 Total de achados: {total_matches}")
    print(f"{'='*60}")
